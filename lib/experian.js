...
var createSocksProxyAgent = require("socks-proxy-agent");
...
var businessOwners = require("./businessOwners");
 ...
 * @param {boolean} [proxyUrl=""] use a proxy url for requests
 * @returns {Experian} Experian API Instance
 */
function Experian(clientId, clientSecret, version, sandbox, proxyUrl) {
  if (!(this instanceof Experian)) {
    return new Experian(clientId, clientSecret, version, sandbox, proxyUrl);
  }
...
Experian.prototype = {
  ...
  login: function (username, password) {
    ...
    var agent;
    if (this._api.proxyUrl) {
      agent = createSocksProxyAgent(this._api.proxyUrl);
    }
